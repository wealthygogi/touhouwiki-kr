...
const INPUT_PATH = path.join(ROOT_DIR, "data", "touhou_list_v2.csv");
const OUTPUT_PATH = path.join(ROOT_DIR, "data", "touhou_normalized_v2.json");
...
function slugifyAsciiId(s) {
  const str = String(s ?? "").normalize("NFKC").toLowerCase();
...
function parseTouhouListV2(text) {
  const rowsRaw = parseCsv(text);
  if (rowsRaw.length === 0) return [];

  const header = rowsRaw[0].map((h) => String(h || "").replace(/\uFEFF/g, "").trim());
  const expected = [
    "Major_Category_en",
    "Sub_Category_en",
    "Major_Category_ja",
  ...
  if (!headerOk) {
    throw new Error(`Unexpected header in ${INPUT_PATH}: ${JSON.stringify(header)}`);
  }
  ...
  for (let i = 1; i < rowsRaw.length; i += 1) {
    ...
    if (cols.length < expected.length) {
      throw new Error(
        `Bad row at line ${i + 1}: expected ${expected.length} columns, got ${cols.length}`
      );
    }
...
function build() {
  ...
  const rows = parseTouhouListV2(input);
  ...
  function ensureWork({
    ...
    const work = {
      ...
      source: {
        file: "data/touhou_list_v2.csv",
        line: typeof source_line === "number" ? source_line : null,
      },
    };
  ...
  for (const r of rows) {
    ...
    // Use the subcategory as the work name.
    ensureWork({
      name_ko: r.sub_ko || "",
      name_ja: r.sub_ja,
      name_en: r.sub_en || r.sub_ja,
  ...
  for (const r of rows) {
    ...
    ensureWork({
      name_ko: koOverride || r.sub_ko || "",
      name_ja: sub,
      name_en: r.sub_en || sub,
  ...
  for (const r of rows) {
    if (r.major_ja === "人妖部門") {
      ...
      characters.push({
        ...
        group_major_ko: r.major_ko || "",
        group_major_en: r.major_en || "",
        group_sub_ja: r.sub_ja,
        group_sub_ko: r.sub_ko || "",
        group_sub_en: r.sub_en || "",
        search: Array.from(
          new Set(
            [
              id,
  ...
  const out = {
    meta: {
      source: "data/touhou_list_v2.csv",
