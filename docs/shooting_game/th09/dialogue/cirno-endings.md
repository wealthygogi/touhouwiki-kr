---
title: "치르노 엔딩"
sidebar_position: 11
description: "치르노의 엔딩 스크립트를 정리했습니다."
---

> 데이터 출처: [Touhou Patch Center — Th09/치르노 엔딩](https://www.thpatch.net/wiki/th09/Cirno%27s_Endings/ko). <br />
> 원문은 CC BY-SA 4.0을 따르며, 동일한 라이선스로 재배포했습니다.

## 치르노 엔딩

|  |  |
| --- | --- |
|  | `8`산골짜기에 있고 그리 크지 않으며 이름도 없는 연못. |
|  | `10`사람의 기척은 전혀 없이 조용하고 고요하게 바람만이 불고 있었다. |
|  | `12`어째서인지 연꽃만이 물방울을 떨어뜨려, 투명도가 높은 물에 파문을 일으키고 있었다. |
|  | `16`&lt;tl$아야:&gt; "오늘은 개구리 안 괴롭히시나요?" |
|  | `19`&lt;tl$치르노:&gt; "다 알고 있는 주제에~" |
|  | `22`&lt;tl$아야:&gt; "학습하셨군요. |
|  | `24`&lt;l$&gt; 이 연못에는 거대 두꺼비가 살고 있다는 사실을요." |
|  | `27`&lt;tl$치르노:&gt; "한번 꿀꺽! 삼켜지면 싫어도 기억하게 된다고." |
|  | `30`&lt;tl$아야:&gt; "그러면 오늘은 왜 이곳을 방문하신 겁니까?" |
|  | `33`&lt;tl$치르노:&gt; "그냥 나도 모르게. |
|  | `35`&lt;l$&gt; 여긴 조용하기도 하고, 유령들이 가끔 날고 있을 뿐 그 외엔 아무것도 없으니깐." |
|  | `38`&lt;tl$아야:&gt; "감상적인 기분이라도 되었다는 말씀인가요? |
|  | `40`&lt;l$&gt; 요정이신 주제에." |
|  | `44`&lt;tl$치르노:&gt; "요정 주제라는 건 무슨 소리얏!! |
|  | `46`&lt;l$&gt; 나도 생각을 하는 경우는 있다고!" |
|  | `49`&lt;tl$아야:&gt; "그야 있으시겠지요. |
|  | `51`&lt;l$&gt; 어제 저녁밥으로 뭘 먹었더라? 라든가." |
|  | `54`&lt;tl$치르노:&gt; "으~응, 뭐였더라? 아니지 그건 상관 없어. |
|  | `56`&lt;l$&gt; 이래봬도 일단 죽음에 대해 생각해 보고 있다니깐." |
|  | `59`&lt;tl$아야:&gt; "죽음이라니, 이거이거 또 희한한 말씀을 하십니다. |
|  | `61`&lt;l$&gt; 누구한테 또 무슨 이야기를 들으셨는지." |
|  | `64`&lt;tl$치르노:&gt; "여기 연못은 조용하고, &lt;l$&gt; 연꽃들도 왠지 이 세상의 것이 아닌 듯한 분위기를 풍기더란 말이야. |
|  | `67`&lt;l$&gt; 그래서 여기서 생각을 하구 있었다고. 죽음에 대해." |
|  | `70`&lt;tl$아야:&gt; "감상적인 기분이 되기에는 아직 이르십니다. |
|  | `72`&lt;l$&gt; 그래봤자 꽃은 아직도 계속 피어나고 있는 중이지요. |
|  | `74`&lt;l$&gt; 지금부터 그렇게 감상에 젖으셨다가 꽃이 질 때는 어쩌시게요? |
|  | `76`&lt;l$&gt; 그야말로 목이라도 매고 싶어질지 모르겠습니다." |
|  | `79`&lt;tl$치르노:&gt; "...... 맞아. |
|  | `81`&lt;l$&gt; 언제까지 피어있을지도 모르니, 즐기지 않으면 손해겠네! |
|  | `83`&lt;l$&gt; 자, 개구리라도 얼리러 가 볼까!" |
|  | `86`&lt;tl$아야:&gt; (두근두근) |
|  | `88`꽃이 언제까지 피어있을지 모르는 것과 마찬가지로, 요정도 언제까지 이 세상에 있을 수 있는지는 알 수 없다. |
|  | `91`꽃이 마음껏 마음껏 피고 있다면 또한 요정들도 마음껏 마음껏 즐거워 하는 것이 좋으리라. |
|  | `94`자연의 죽음이라는 것은 대체 무엇인가. 치르노는 별로 이해할 수 없었다. |
|  | `96`그 말에서는 인간에 의한 자연파괴와는 전혀 다른, 더욱 허무한 느낌이 전해져 왔다. |
|  | `98`인간이 죽으면 지고 있는 죄에 따라 지옥이나 천계로 갈 장소가 나뉘어지게 된다. 경우에 따라서는 윤회전생을 하게 되는 경우도 있겠지. |
|  | `101`자연의 경우에는 어떨 것인가. 전생을 한다면 아무 문제도 없다. |
|  | `103`허나, 혹시 지옥에 떨어지게 된다면 자연에 속하는 요정은 어떻게 될까? |
|  | `105`치르노는 무서운 상상을 하게 되자, 거기서 생각하는 것을 중단하는 것이었다. |
|  | `107`&lt;c$Congratulations! Ending No.6$&gt; |
